...
from sales.models import SalesOrder, SalesOrderLine, DeliveryNote, DeliveryLine, SalesPickup, SalesPickupLine
...
class SalesPickupLineSerializer(serializers.ModelSerializer):
    item_code = serializers.CharField(source='item.code', read_only=True)
    item_name = serializers.CharField(source='item.name', read_only=True)
    unit_abbr = serializers.CharField(source='unit.abbreviation', read_only=True)
...
class SalesPickupSerializer(serializers.ModelSerializer):
    lines = SalesPickupLineSerializer(many=True, read_only=True)
    customer_name = serializers.CharField(source='customer.name', read_only=True)
    warehouse_name = serializers.CharField(source='warehouse.name', read_only=True)
    created_by_name = serializers.CharField(source='created_by.get_full_name', read_only=True)
    ...
    class Meta:
        ...
        fields = [
            'id', 'document_number', 'status',
            'sales_order', 'customer', 'customer_name',
            'warehouse', 'warehouse_name', 'pickup_date',
            'pickup_by', 'notes',
            'created_by', 'created_by_name',
            'approved_by', 'approved_at', 'posted_by', 'posted_at',
            'created_at', 'lines',
